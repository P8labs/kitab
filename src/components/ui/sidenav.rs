        ...
        {if data_collapsible == SidenavCollapsible::None {
            view! {
                <aside
                    data-name="Sidenav"
                    class=tw_merge!(
                        "flex flex-col h-full bg-transparent text-sidenav-foreground w-(--sidenav-width)", class
                    )
                >
                    {children()}
                </aside>
            ...
            view! {
                <aside
                    ...
                    class="hidden md:block group peer text-sidenav-foreground data-[state=Collapsed]:hidden bg-transparent"
                >
                    ...
                    <div
                        ...
                        class=tw_merge!(
                            "fixed inset-y-0 z-10 hidden h-svh w-(--sidenav-width) transition-[left,right,width] duration-200 ease-linear md:flex bg-transparent!",
                            class,
                            match data_side {
...
pub fn SidenavTrigger(children: Children) -> impl IntoView {
    ...
    view! {
        <Button
            class="size-6 bg-background ml-2 mt-0.5" variant=ButtonVariant::Accent size=ButtonSize::Icon
...
fn SidenavToggleRail() -> impl IntoView {
    ...
    view! {
        <button
            ...
            class="hidden absolute inset-y-0 z-20 w-4 transition-all ease-linear -translate-x-1/2 sm:flex group-data-[side=Left]:-right-4 group-data-[side=Right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 in-data-[side=Left]:cursor-w-resize in-data-[side=Right]:cursor-e-resize [[data-side=Left][data-state=Collapsed]_&]:cursor-e-resize [[data-side=Right][data-state=Collapsed]_&]:cursor-w-resize group-data-[collapsible=Offcanvas]:translate-x-0 group-data-[collapsible=Offcanvas]:after:left-full [[data-side=Left][data-collapsible=Offcanvas]_&]:-right-2 [[data-side=Right][data-collapsible=Offcanvas]_&]:-left-0eft-2 hover:after:bg-sidenav-border hover:group-data-[collapsible=Offcanvas]:bg-sidenav"
        />
    }
}
